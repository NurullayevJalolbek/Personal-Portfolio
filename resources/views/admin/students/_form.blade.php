...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-style1">
            ...
            <li class="breadcrumb-item">
                <a href="{{ route('admin.students.index') }}">{{ __('admin.customers') }}</a>
            </li>
        ...
        <div class="card-body">
            <form action="{{ $route }}" method="POST" id="formValidationExamples" class="row g-3"
                ...
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <div class="row g-2">
                            ...
                            <div class="col-md-6">
                                <x-form.label-input name="email" label="{{ __('admin.Email') }}"
                            ...
                            <div class="col-md-6">
                                <x-form.label-select name="country_id" label="{{ __('admin.Country') }}">
                                    <option value="">{{ __('admin.Select') }}</option>
                                    @foreach ($countries as $item)
                                        <option value="{{ $item->id }}" @selected((old('country_id') ?? $model->country_id) == $item->id)>
                                            {{ $item->translate('name') }}
                                        </option>
                                    @endforeach
                                </x-form.label-select>
                            ...
                            <div class="col-md-6">
                                <x-form.label-select name="program_id" label="{{ __('admin.Program') }}">
                                    <option value="">{{ __('admin.Select') }}</option>
                                    @foreach ($programs as $item)
                                        <option value="{{ $item->id }}" @selected((old('program_id') ?? $model->program_id) == $item->id)>
                                            {{ $item->translate('name') }}
                                        </option>
                                    @endforeach
                                </x-form.label-select>
                ...
                <div class="col-12 d-flex justify-content-between">
                    <a href="{{ route('admin.students.index') }}" class="btn btn-secondary">{{ __('admin.Back') }}</a>
